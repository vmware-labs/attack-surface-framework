...
from app.discovery import internal_delete

#Static and Global Declarations

def parser_default(PARSER_INPUT, PARSER_OUTPUT, tag, mode, owner):
    for DATA in PARSER_INPUT:
        debug(str(DATA))
    return

def parser_vmw_csv(PARSER_INPUT, PARSER_OUTPUT, tag, mode, owner):
...
def parser_crt_sh(PARSER_INPUT, PARSER_OUTPUT, tag, mode, owner):
    for DATA in PARSER_INPUT:
        DATA=DATA.rstrip()
        debug("\tDomain:"+DATA+"\n")
        debug("\t\tOwners:"+owner+"\n")
...
def parser_vmw_json(PARSER_INPUT, PARSER_OUTPUT, tag, mode, owner):
    ...
    for SECTION in JSON_DATA:
        ...
        for DOMAIN in JSON_DATA[SECTION][0]:
            ...
            UNIT['tag'] = str(JSON_DATA[SECTION][2])
...
def parser_vmw_jsonl(PARSER_INPUT, PARSER_OUTPUT, tag, mode, owner):
...
action={'default':parser_default, 'vmw.csv':parser_vmw_csv, 'vmw.json':parser_vmw_json, 'vmw.jsonl':parser_vmw_jsonl, 'crt.sh':parser_crt_sh}
...
def discovery_manager(JSONL_FILE, Tag, WorkMode):
    ...
    for JSONL_DATA in JSONL_FILE:
        ...
        try:
            Answer = vdResult.objects.update_or_create(name=domain, defaults={'type': Type, 'tag':Tag, 'lastdate': LastDate, 'owner': data['owner'], 'metadata': JSONL_DATA})
            if Answer[1]:
